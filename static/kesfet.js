...
// Konu anahtarları (gundem, sanat…) kullanıcıya akış ve İçgörü'dekiyle aynı adlarla gösterilir.
const KONU_ADI={gundem:"Gündem",teknoloji:"Teknoloji",bilim:"Bilim",spor:"Spor",sanat:"Kültür",saglik:"Yaşam",ekonomi:"Ekonomi",egitim:"Eğitim",oyun:"Oyun",seyahat:"Keşif"};
const konuAdi=konu=>KONU_ADI[konu]||konu;
...
function openExploreReaction(post,trigger){
  ...
  exploreReactionTray.querySelectorAll("[data-explore-reaction]").forEach(button=>button.addEventListener("click",async event=>{event.preventDefault();event.stopPropagation();const current=activeExploreReaction;if(!current)return;const reaction=button.dataset.exploreReaction;exploreReactions[current.post.id]=reaction;current.trigger.innerHTML=`${exploreReactionLabels[reaction]}`;closeExploreReaction();if(window.LocalPersonalization)await window.LocalPersonalization.recordPostReaction(current.post,reaction);}));
}
...
function card(post,index){const [glyph,background,shape]=styleFor(post.konu);const variant=index%7===0?" feature":index%5===0?" tall":"";return `<button class="explore-card${variant}" data-id="${post.id}" style="--explore-bg:${background};--explore-shape:${shape}"><span class="explore-glyph">${glyph}</span><span class="explore-topic">${safe(konuAdi(post.konu))}</span><span class="explore-card-text">${safe(post.metin)}</span><span class="explore-card-meta">${index%3===0?"▶  Keşfet":"✦  Senin için"}</span></button>`;}
function render(){const items=visiblePosts();grid.innerHTML=items.length?items.map(card).join(""):'<div class="explore-empty"><span><svg class="ikon ikon-lg" aria-hidden="true"><use href="#i-kesfet"/></svg></span><b>Burada henüz bir şey yok</b><p>Başka bir konu ya da arama deneyebilirsin.</p></div>';grid.querySelectorAll(".explore-card").forEach(button=>button.addEventListener("click",()=>openPost(posts.find(post=>post.id===Number(button.dataset.id)))));}
function openPost(post){if(!post)return;const [glyph,background]=styleFor(post.konu);document.getElementById("explore-modal-content").innerHTML=`<div class="modal-visual" style="background:${background}"><span>${glyph}</span></div><span class="eyebrow">${safe(konuAdi(post.konu))} · KEŞFET</span><h2>${safe(post.metin)}</h2><p>Bu içerik ilgi alanların ve akışındaki çeşitlilik için seçildi.</p><div class="modal-actions"><button id="explore-reaction-trigger" class="explore-reaction-trigger" type="button" aria-expanded="false">☺ ${exploreReactions[post.id]?exploreReactionLabels[exploreReactions[post.id]]:"Tepkiler"}</button><button type="button">↗ Paylaş</button><a href="/index.html">Akışa dön <svg class="ikon ikon-sm" aria-hidden="true"><use href="#i-ok-sag"/></svg></a></div>`;const modal=document.getElementById("explore-modal");modal.classList.add("open");modal.setAttribute("aria-hidden","false");document.getElementById("explore-reaction-trigger").addEventListener("click",event=>{event.preventDefault();event.stopPropagation();openExploreReaction(post,event.currentTarget);});}
...
render=function(){
  ...
  grid.querySelectorAll(".explore-card").forEach(button=>{
    ...
    if(post)button.insertAdjacentHTML("afterbegin",`<img class="explore-photo" src="${postImage(post)}" alt="${safe(konuAdi(post.konu))} içeriği için temsili görsel" loading="lazy">`);
  });
};
...
openPost=function(post){
  ...
  if(visual)visual.innerHTML=`<img src="${postImage(post)}" alt="${safe(konuAdi(post.konu))} içeriği için temsili görsel">`;
};
