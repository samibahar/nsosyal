...
async function load(){grid.innerHTML='<div class="loading-state">Keşif hazırlanıyor…</div>';try{const response=await fetch("/api/kesfet");posts=(await response.json()).gonderiler;render();}catch{grid.innerHTML='<div class="explore-empty"><b>Keşif şu an yüklenemedi.</b><p>Bağlantıyı kontrol edip tekrar dene.</p></div>';}}
