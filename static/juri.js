...
function renderTrace(trace) {
  ...
  const dengeleme = trace.dengelemeAcik === false
    ? "Kullanıcı duygu dengelemesini Ayarlar'dan kapattığı için hiçbir adaya uygulanmadı; sıra yalnızca ilgi, çeşitlilik ve açık tepkiyle belirlendi."
    : `${balanced} yoğun adayın sırası, sürdürülebilir örüntü oluştuğunda içerik silinmeden nazikçe azaltıldı.`;
  document.getElementById("jury-receipt").innerHTML = `<p><b>İlgi:</b> Hazır senaryodaki açık etkileşimler ilgili konuların yerel ağırlığını güncelledi.</p><p><b>Dengeleme:</b> ${dengeleme}</p><p><b>Kontrol:</b> Son açık tepki “Gerildim” olarak kaydedildi; bu bir teşhis değil, kullanıcının gönüllü bildirimi.</p><p><b>Gizlilik:</b> Karar izi IndexedDB’de bu cihazda tutulur. Sunucu yalnızca örnek adayları sağladı.</p>`;
